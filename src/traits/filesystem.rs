...
pub mod tests {
    ...
    impl FileSystem for MockFileSystem {
        async fn exists(&self, path: &str) -> DottResult<bool> {
            ...
            Ok(files.contains_key(path) || dirs.iter().any(|p| p == path) || symlinks.contains_key(path))
        }
        ...
        async fn create_symlink(&self, source: &str, target: &str) -> DottResult<()> {
            self.symlinks.lock().unwrap().insert(target.to_string(), source.to_string());
            Ok(())
        }
        ...
        async fn copy_file(&self, source: &str, target: &str) -> DottResult<()> {
            ...
            if let Some(content) = content {
                self.files.lock().unwrap().insert(target.to_string(), content);
            }
            Ok(())
        }
        ...
        async fn read_to_string(&self, path: &str) -> DottResult<String> {
            self.files
                ...
                .get(path)
                .cloned()
                .ok_or_else(|| crate::error::DottError::Io(
...
mod filesystem_tests {
    ...
    async fn test_mock_filesystem_file_operations() {
        ...
        assert_eq!(fs.read_to_string("test.txt").await.unwrap(), "Hello, world!");
    ...
    async fn test_mock_filesystem_symlink_operations() {
        ...
        fs.create_symlink("/source/file", "/target/link").await.unwrap();
