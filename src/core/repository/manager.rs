...
mod tests {
    ...
    async fn test_repository_manager_validate_and_fetch() {
        ...
        let config = manager.validate_and_fetch_config("https://github.com/test/repo.git")
            .await
            .unwrap();
    ...
    async fn test_repository_manager_clone() {
        ...
        manager.clone_repository("https://github.com/test/repo.git", "/tmp/repo")
            .await
            .unwrap();
    ...
    async fn test_repository_manager_status() {
        ...
        let manager = RepositoryManager::new(mock_repo);
        let status = manager.get_repository_status("/tmp/repo")
            .await
            .unwrap();
