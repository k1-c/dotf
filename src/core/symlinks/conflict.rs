...
use super::backup::{BackupManager, BackupEntry};
...
impl<F: FileSystem + Clone, P: Prompt> ConflictResolver<F, P> {
    ...
    pub async fn check_conflict(&self, source_path: &str, target_path: &str) -> DottResult<Option<ConflictInfo>> {
        ...
        let existing_target = if existing_is_symlink {
            Some(self.filesystem.read_link(target_path).await?.to_string_lossy().to_string())
        } else {
            None
        };
    ...
    pub async fn resolve_conflict(
        ...
        match resolution {
            ...
            ConflictResolution::Abort => {
                Err(DottError::Operation("Operation aborted by user".to_string()))
            }
            ...
            ConflictResolution::Backup => {
                let backup_entry = self.backup_manager.backup_file(&conflict.target_path).await?;
                self.remove_existing(&conflict.target_path).await?;
                self.backup_manager.add_backup_entry(backup_entry.clone()).await?;
                Ok(Some(backup_entry))
            }
        }
    }

    pub async fn resolve_conflict_interactive(&self, conflict: &ConflictInfo) -> DottResult<Option<BackupEntry>> {
        let existing_type = if conflict.existing_is_symlink {
            format!("symlink -> {}", conflict.existing_target.as_ref().unwrap_or(&"unknown".to_string()))
        } else {
            "file".to_string()
        };
        ...
        let options = vec![
            ...
            ("Backup", "Backup existing file/symlink and create new symlink"),
            ("Overwrite", "Remove existing file/symlink and create new symlink"),
    ...
    pub async fn resolve_all_conflicts_interactive(
        ...
        let options = vec![
            ...
            ("Backup All", "Backup all existing files and create symlinks"),
            ("Overwrite All", "Overwrite all existing files with symlinks"),
        ...
        match choice {
            ...
            2 => {
                ...
                let mut backup_entries = Vec::new();
                for conflict in conflicts {
                    if let Some(entry) = self.resolve_conflict(conflict, ConflictResolution::Backup).await? {
                        backup_entries.push(entry);
                    }
                }
                Ok(backup_entries)
            ...
            3 => {
                // Overwrite all
                for conflict in conflicts {
                    self.resolve_conflict(conflict, ConflictResolution::Overwrite).await?;
                }
                Ok(Vec::new())
            }
            4 | _ => {
...
mod tests {
    ...
    async fn test_no_conflict_when_file_does_not_exist() {
        ...
        let conflict = resolver.check_conflict("/source/.vimrc", "/home/user/.vimrc").await.unwrap();
    ...
    async fn test_conflict_with_existing_symlink() {
        ...
        fs.create_symlink("/other/source/.vimrc", "/home/user/.vimrc").await.unwrap();
        ...
        assert_eq!(conflict.source_path, "/source/.vimrc");
        assert!(conflict.existing_is_symlink);
        assert_eq!(conflict.existing_target, Some("/other/source/.vimrc".to_string()));
    }
    ...
    async fn test_resolve_conflict_skip() {
        ...
        let result = resolver.resolve_conflict(&conflict, ConflictResolution::Skip).await.unwrap();
    ...
    async fn test_resolve_conflict_overwrite() {
        ...
        let result = resolver.resolve_conflict(&conflict, ConflictResolution::Overwrite).await.unwrap();
        assert!(result.is_none());
    ...
    async fn test_resolve_conflict_backup() {
        ...
        let result = resolver.resolve_conflict(&conflict, ConflictResolution::Backup).await.unwrap();
    ...
    async fn test_resolve_conflict_abort() {
        ...
        let result = resolver.resolve_conflict(&conflict, ConflictResolution::Abort).await;
